...
function DetalheDesenho() {
  ...
  // Trava por senha: Barbie: Life in the Dreamhouse tem acesso próprio e
  // independente das demais seções. Nunca renderiza o conteúdo antes de
  // confirmar (via localStorage) que o acesso já foi liberado.
  if (ehBarbieDreamhouse && !barbieLiberado) {
    if (!barbieChecado) return null;
    return (
      <BarbieDreamhouseGate
        bannerUrl={desenho.coverHd}
        onFechar={() => navigate({ to: "/" })}
